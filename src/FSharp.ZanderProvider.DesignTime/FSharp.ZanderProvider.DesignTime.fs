module FSharp.ZanderProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("FSharp.ZanderProvider.DesignTime", "FSharp.ZanderProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("BasicGenerativeProvider.DesignTime", "FSharp.ZanderProvider")])

    let ns = "FSharp.ZanderProvider"
    let asm = Assembly.GetExecutingAssembly()
